...
def ctn_metric(config: Config):
    ...
    for result in response:
        ...
        prom_memory_sql = 'sum(container_memory_working_set_bytes{namespace=\'%s\',pod="%s"}) by(pod, instance)  / 1000000' % (config.namespace, pod_name)
        prom_network_sql =  'sum(rate(container_network_transmit_packets_total{namespace=\"%s\", pod="%s"}[1m])) * sum(rate(container_network_transmit_packets_total{namespace=\"%s\", pod="%s"}[1m]))' % (config.namespace, pod_name, config.namespace, pod_name),
...
def node_metric(config: Config):
    ...
    for node in config.nodes.values():
        ...
        response = executeProm(config, prom_sql)
        values = response[0]['values']     
        values = list(zip(*values))
        metric = pd.Series(values[1])
        ...
        prom_sql =  '1-(sum(increase(node_cpu_seconds_total{instance="%s",mode="idle"}[1m]))/sum(increase(node_cpu_seconds_total{instance="%s"}[1m])))' % (node, node)
        response = executeProm(config, prom_sql)
        values = response[0]['values']
        values = list(zip(*values))
        metric = pd.Series(values[1])
        ...
        prom_sql =  '(node_memory_MemTotal_bytes{instance="%s"}-(node_memory_MemFree_bytes{instance="%s"}+ node_memory_Cached_bytes{instance="%s"} + node_memory_Buffers_bytes{instance="%s"})) / node_memory_MemTotal_bytes{instance="%s"}' % (node,node,node,node,node)
        response = executeProm(config, prom_sql)
        values = response[0]['values']
        values = list(zip(*values))
        metric = pd.Series(values[1])
